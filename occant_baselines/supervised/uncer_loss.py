...
class DiceLoss(nn.Module):
    ...
    def forward(self, preds, targets, weight=False):
        ...
        # 这里是根据真实类别来设置对应pred通道为1，其他为0
        # gt是单通道的就是一个label，而pred是多通道的对应着每个通道的概率
...
class NIGLoss(nn.Module):
    ...
    @classmethod
    def init(cls, lam=0.0, epsilon=1e-2, maxi_rate=1e-4):
        return cls(lam, epsilon, maxi_rate)
    ...
    def forward(self, mu, alpha, v, beta, y, reduce=True, return_comps=False):
        
        nll_loss = self._nig_nll(y, mu, v, alpha, beta, reduce=reduce)
